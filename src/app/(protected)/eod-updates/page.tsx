...
export default async function EodUpdatesPage() {
  ...
  const currentDeveloper = developers[0] ?? null
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  {session.role === 'admin' ? (
                    ...
                    <>
                      <input type="hidden" name="developerId" value={currentDeveloper?.id ?? ''} />
                      <input value={currentDeveloper?.name ?? 'Unassigned developer'} disabled className="mt-1 block h-10 w-full rounded-md border border-gray-300 bg-gray-50 px-3 text-sm" />
                    </>
                  )}
                </div>
                <div>
